# TODO

- [x] generate static website as Nix output, using https://zensical.org
- [ ] host generated website on aifiles.vorburger.ch
- [ ] move MD checks feom aifiles to nixfiles
- [x] Validate skills, e.g. with [skills-ref](https://github.com/agentskills/agentskills/tree/main/skills-ref#cli)
- [ ] Investigate if `lychee` caches external link validation results.
  - [ ] Determine how to activate and configure caching for 24 hours.
  - [ ] If `lychee` doesn't support 24h caching, file a Feature Request (FR).
  - [ ] Search for and find an alternative tool that supports 24h caching of external link validation if `lychee` does not.
- [ ] Eval (soe of the) skills, using [skill-creator](https://github.com/anthropics/skills/tree/main/skills/skill-creator), see https://agentskills.io/skill-creation/evaluating-skills.
- [ ] Interesting skills to try out
  - [ ] https://skills.sh/vercel-labs/skills/find-skills
  - [ ] https://skills.sh/coreyhaines31/marketingskills/seo-audit
  - [ ] https://skills.sh/browser-use/browser-use/browser-use
- [ ] Turn https://docs.enola.dev/agents/git-commit/ into a skill here
- [ ] Turn https://docs.enola.dev/agents/github-issue/ into a skill here
